...
# SequentialLR calls step() on sub-schedulers during __init__ before any
# optimizer.step() — the order in optimizer_and_scheduler_step is correct;
# this warning is a false positive from the scheduler initialization.
warnings.filterwarnings(
    "ignore",
...
sys.path.append("/home/leonardo/Desktop/Repos/PoseBench/benchmarks_3D")
from benchmark_pose import (
    eval_colmap_model,
    evaluate_scene
)

def plot_image_and_depth(img_tensor, depth_tensor):
    # Convert to CPU and numpy if they are torch tensors
    ...
    # Plot RGB Image
    ax[0].imshow(img)
    ax[0].set_title("Original Image (00571.png)")
    ax[0].axis("off")

    # Plot Depth Map with a colormap (e.g., 'magma' or 'viridis')
    im2 = ax[1].imshow(depth, cmap='magma')
    ax[1].set_title("Depth Map")
    ax[1].axis("off")
    
    # Add a colorbar to help interpret depth values
    plt.colorbar(im2, ax=ax[1], label='Distance / Intensity')
...
class Adjuster(nn.Module, MiscModule, ReconstructAndVizModule):
    ...
    Args:
        ...
        n_mapping_frames (int, optional): Number of frames from the reconstruction to use for the mapping (initialization)
            phase. The remaining frames are fed one-by-one via add_frame / forward_frame during tracking.
            If None, all frames in the reconstruction are used for mapping. Default is None.
    """

    def __init__(
        self,
        reconstruction_path,                        #Path to the colmap like reconstruction data
        images_path,                                #Path to images
        depths_path,                                #Path to depths
        viewgraph_path=None,                        #For testing with GT viewgraph
        unreliable_area_masks_path=None,            
        images_size=518,                            # 
        single_camera_per_folder=True,
        load_with_pad=False,
        rescale = False,                            # rescale image and depth to image_size
        detector="canny",
        device="cuda",
        max_workers=-1,
        detector_params={
        ...
        scene_type="indoor",  # or "outdoor", "object_centric" (not used yet)
        ...
        self.rescale = rescale                      #Rescale image, depths and camera
        ...
        # Determine how many frames to use for the mapping (initialization) phase
        _n_recon = len(self.recon.images)
        self.n_mapping_frames = n_mapping_frames if n_mapping_frames is not None else _n_recon
        ...
        self.image_path_list = find_images(
            self.images_path
        )[:self.n_mapping_frames]  # image name includes subfolder if any
        ...
        ########################################3
        self.rms_ate_list = {"rms_ate": [], "steps": []}
    ...
    def forward(
        ...
        refinement = False,              #list of view to optimize
        refinement_iteration = 200       #Number of iteration to refine
    ):
        """
        ...
        #   # Selective updates
        #   # If selective update the viewgraph involves just the pairs in the list
        #   if len(indices) !=0: 
        #   
        #   else:   
        ...
        if(refinement): 
            ...
            self.max_num_iterations = refinement_iteration

            self.q_lr = 3e-3
            self.t_lr = 3e-3
            self.k_lr = 3e-3
            self.z_lr = 3e-3
            self.mlp_pose_lr = 3e-3

            if self.use_mlp_pose_refinement and hasattr(self.poses, "mlp"):
                self.poses.mlp._init_weights()
                self.poses.init_optimizer(mlp_lr=self.mlp_pose_lr)
        ...
        for step in bar:
            ...
            if gt_path is not None and step % self.auc_saving_freq == 0:
                ...
                self.compute_rsm_ate(opt, gt_path, step)
                #self.compute_error(opt, gt_path, step)

            if self.verbose:
                bar.set_postfix(
                    loss=f"{self.loss_list[-1]:.4f}",
                    rmse=f"{self.rms_ate_list['rms_ate'][-1]:.4f}",
                    auc5=(
    ...
    def _read_cameras_from_reconstruction(self):
        ...
        if self.single_camera_per_folder:
            ...
            for image in self.recon.images.values():
                _, model, new_params = process_camera(
                    image.camera, self.load_with_pad, images_size=self.images_size, rescale=self.rescale
                )
        ...
        poses_temp = {}
        for image in sorted(self.recon.images.values(), key=lambda img: img.name)[:self.n_mapping_frames]:
    ...
    @torch.no_grad()
    def _compute_viewgraph(self, image_names = [], type="exhaustive"):
        ...
        if self.viewgraph_path is not None:
            ...
            if len(image_names) == 0: image_names = sorted(list(self.images.keys()))
            else: image_names = image_names
            viewgraph = []
...
####################################################################################################################################
    ...
    def align_trajectories(self,traj_pred, traj_gt):
        """
    ...
    def get_camera_centers(self, reconstruction):
        ...
        for img_id in img_ids:
            ...
            if not image.has_pose or image.cam_from_world is None:
                continue
    ...
    def compute_rsm_ate(self, opt, gt_path, step):
        pred_recon = pycolmap.Reconstruction(opt)
        gt_recon = pycolmap.Reconstruction(gt_path)
        pred_names = {img.name for img in pred_recon.images.values()}
        for img_id in list(gt_recon.images.keys()):
            if gt_recon.images[img_id].name not in pred_names:
                gt_recon.deregister_image(img_id)

        traj_pred = self.get_camera_centers(pred_recon)
        traj_gt = self.get_camera_centers(gt_recon)

        # --- NEW: Align before computing error ---
        traj_pred_aligned = self.align_trajectories(traj_pred, traj_gt)
        ...
        self.rms_ate_list["rms_ate"].append(rms_ate)
        self.rms_ate_list["steps"].append(step)
        return rms_ate

    def compute_rsm_evo(self, opt, gt_path, step):
    ...
    def _create_frame(
        ...
        # --- Image ---
        image_name, img_tensor, coords, scale = _process_single_image(
            image_path,
            images_path,
            self.images_size,
            TF.ToTensor(),
        ...
        self.images[image_name] = {
            "image":  img_tensor.to(self.device, dtype=dtype),
        ...
        # --- Depth ---
        _, depth_tensor, confidence_tensor = _process_single_depth(
            depths_path,
            image_name,
            self.images[image_name],
            self.images_size,
        ...
        if depth_tensor is not None:
            self.images[image_name]["depth"] = depth_tensor.to(device, dtype=dtype)
        else:
            raise ValueError(f"Depth map not found for {image_name}. A depth map is required for pose inference.")

        if confidence_tensor is not None:
            self.images[image_name]["confidence"] = confidence_tensor.to(device, dtype=dtype)

        new_idx = self.num_images                                           # next available slot
        self.image_id_map[image_name] = new_idx
        self.num_images += 1
        ...
        cam_id = image.name.split("/")[0]
        self.images[image_name]["cam_id"] = cam_id

        self.intrinsics.add_element(cam_id, 0)
        ...
        # ------------------------------------------------------------------ #
        # 2. Compute edges and distance field for the new frame              #
        # ------------------------------------------------------------------ #

        img_tensor_batch = self.images[image_name]["image"].unsqueeze(0)   # (1, C, H, W)
        edges_map = (
            self.edge_extractor(img_tensor_batch)
            .squeeze()
            .to(device, dtype=dtype)
        )

        # Optionally mask low-confidence depth regions
        confidence_threshold = 0.2
        if "confidence" in self.images[image_name] and self.use_depth_confidence:
            conf = self.images[image_name]["confidence"]
            conf_norm = (conf - conf.min()) / (conf.max() - conf.min() + 1e-8)
            valid_mask = (conf_norm > confidence_threshold) & (~torch.isnan(conf_norm))
            edges_map = edges_map * valid_mask.to(dtype)

        valid_edges = edges_map.squeeze().nonzero().flip(dims=(1, 0)).to(device, dtype=dtype)  # (N, 2)

        self.images[image_name]["edges_map"] = edges_map
        self.images[image_name]["edges"]     = valid_edges

        # Pad the new frame's edges to match the current global max_edges budget.
        n_edges = valid_edges.shape[0]
        if n_edges >= self.max_edges:
            idx    = torch.randperm(n_edges, device=device)[:self.max_edges]
            edges_padded = valid_edges[idx].to(dtype)
            pad_mask     = torch.ones(self.max_edges, device=device, dtype=torch.bool)
        else:
            pad_size     = self.max_edges - n_edges
            pad_zeros    = torch.zeros((pad_size, 2), device=device, dtype=dtype)
            edges_padded = torch.cat([valid_edges, pad_zeros], dim=0)
            pad_mask     = torch.zeros(self.max_edges, device=device, dtype=torch.bool)
            pad_mask[:n_edges] = True

        self.images[image_name]["edges_padded"] = edges_padded
        self.images[image_name]["pad_mask"]     = pad_mask

        # Sample depth at edge locations
        sampled_depth, _ = grid_sample_nan(edges_padded[None], self.images[image_name]["depth"][None])
        sampled_depth = torch.where(
            torch.isnan(sampled_depth),
            torch.full_like(sampled_depth, 1e-6),
            sampled_depth,
        )
        self.images[image_name]["sampled_depth"] = sampled_depth.squeeze()

        # Distance transform for this frame
        dt_field = compute_distance_field_cv2(edges_map, device=device).to(device, dtype=dtype)

        # Pad the DT field to match existing frames if shapes differ
        existing_dt_shape = self.dt_fields.params.shape[-2:]           # (H, W)
        h_dt, w_dt = dt_field.shape
        if h_dt < existing_dt_shape[0] or w_dt < existing_dt_shape[1]:
            pad_b = existing_dt_shape[0] - h_dt
            pad_r = existing_dt_shape[1] - w_dt
            dt_field = F.pad(dt_field, (0, pad_r, 0, pad_b), mode="constant", value=dt_field.max())

        self.images[image_name]["dt_field"] = dt_field

       
        # # ------------------------------------------------------------------ #
        # # 3. Update the batched parameter modules and viewgraph               #
        # # ------------------------------------------------------------------ #

        # Append to the batched tensor modules (each wraps a (N, ...) parameter tensor)
        self.edges_padded.add_element(image_name, new_idx, edges_padded)
        self.pad_masks.add_element(image_name, new_idx, pad_mask)
        self.dt_fields.add_element(image_name, new_idx, dt_field.unsqueeze(0))       # module stores (1, H, W)
        self.images_hw.add_element(image_name, new_idx, torch.tensor(self.images[image_name]["hw"]))
    ...
    def _update_viewgraph(
        ...
        new_pairs, _ = filter_viewgraph_by_reprojection_batched(
            viewgraph = candidate_pairs,
            images = self.images,
            intrinsics = self.intrinsics,
            poses = self.poses,
            min_points = 50,
        ...
        # Merge new pairs into the viewgraph and adjacency list
        ...
        # Rebuild the integer-index viewgraph tensor used during optimisation
        cam_id_new = self.images[image_name]["cam_id"]
    ...
    def forward_frame(
        ...
        t_frame = nn.Parameter(
            self.poses.t_param[new_frame_idx].detach().clone(),
            requires_grad=True
        )

        local_optimizer = torch.optim.Adam([
            {"params": [q_frame], "lr": q_lr},
            {"params": [t_frame], "lr": t_lr},
        ])

        for _ in tqdm(range(refinement_iteration), desc=f"frame {image_name}", leave=False):
            local_optimizer.zero_grad()

            # Write standalone params into the module
            with torch.no_grad():
                self.poses.q_param.data[new_frame_idx] = q_frame.detach()
                self.poses.t_param.data[new_frame_idx] = t_frame.detach()

            # Bypass MLP entirely for this frame
            ...
            residuals, sampled_viewgraphs = self.compute_forward_step(
                local_viewgraph,
                batch_size=len(local_viewgraph),
                drop_last=False,
                residuals_chunk_size=2048,
            )

            loss = self.compute_batched_loss(residuals, sampled_viewgraphs)
            loss.backward()

            # Manually push gradients back into standalone params
            with torch.no_grad():
                if self.poses.q_param.grad is not None:
                    q_frame.grad = self.poses.q_param.grad[new_frame_idx].clone()
                if self.poses.t_param.grad is not None:
                    t_frame.grad = self.poses.t_param.grad[new_frame_idx].clone()
    ...
    def unproject_edge_to_3D(self, image_name):
        ...
        cam_ids = self.images_cams_ids[image_id, 1]

        K = self.intrinsics.get_intrinsic_matrix([cam_ids]).unsqueeze(0)  # (1, 3, 3)
        P = self.poses.get_projection_matrix([image_id])                  # (1, 4, 4)
        edges = self.edges_padded.get_parameters([image_id])              # (1, N, 2)
        depth = self.sampled_depth.get_parameters([image_id])             # (1, HxW)

        pts3d = unproject_2D_to_world(
            xy0=edges, K0=K, depth0=depth, P0=P
        ).squeeze(0)  # (N, 3)

        # Detach old params to break stale computation graphs from previous iterations,
        # then splice in the fresh differentiable pts3d at image_id.
        # old[image_id+1:] is empty when this frame is new, making this an append.
        old = self.edges_3D.params.detach()
        self.edges_3D.params = torch.cat([
            old[:image_id],
            pts3d.unsqueeze(0),
            old[image_id + 1:],
        ], dim=0)
...
class FrameOptimizer:
    ...
    def _global_refinement(self):
        self.adj(refinement=True, refinement_iteration=self.refinement_iterations,
             early_stop="pose", window_pose=25, convergence_tol_pose=0.5)